...
// Update the getQuizResults function to fix the 500 error
const getQuizResults = async (req, res) => {
  try {
    const { id_student, id_quiz, page = 1, limit = 1 } = req.query;
    console.log("Quiz results request:", id_student, id_quiz, page, limit);
    ...
    if (isNaN(studentId) || isNaN(quizId)) {
      return res.status(400).json({ error: "Invalid student ID or quiz ID" });
    }
    
    // Get total count of attempts for pagination
    ...
    // Get paginated attempts - using attempt_at instead of created_at for ordering
    const attempts = await prisma.attempts.findMany({
      ...
      take: limitNum,
      orderBy: {
        // Use attempt_at instead of created_at, or id_attempt if attempt_at is not available
        attempt_at: 'desc'
      },
      include: {
    ...
    // Include quiz details for each attempt
