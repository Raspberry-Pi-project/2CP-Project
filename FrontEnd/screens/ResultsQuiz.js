...
const ResultsQuiz = ({ route, navigation }) => {
    ...
    const renderAnswerItem = ({ item, index }) => {
        return (
            <View 
                key={`answer-${item.id_question}-${index}`}
                style={[styles.answerRow, { backgroundColor: item.correct ? 'rgba(76, 175, 80, 0.1)' : 'rgba(106, 90, 205, 0.05)' }]}
            >
    ...
    const renderAttemptDetails = (attempt) => {
        return (
            <View style={styles.attemptContainer} key={`attempt-${attempt.id_attempt}`}>
                ...
                {attempt.student_answers.map((answer, index) => 
                    renderAnswerItem({ item: answer, index })
                )}
            </View>
        );
    };
