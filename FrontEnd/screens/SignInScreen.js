...
const handleSignIn = async () => {
  try {
    ...
    // Make the API request with a more specific error handling
    const response = await apiClient.post('/auth/login', {
      email,
      password,
    }).catch(error => {
      if (error.code === 'ECONNABORTED') {
        throw new Error('Login request timed out. Please try again or check your connection.');
      } else if (!error.response) {
        throw new Error('Network error. Please check your internet connection.');
      } else if (error.response.status === 401) {
        throw new Error('Invalid email or password');
      } else {
        throw new Error(`Server error (${error.response?.status || 'unknown'}). Please try again later.`);
      }
    });
