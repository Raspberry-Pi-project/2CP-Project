...
const ConfirmDialog = React.memo(({ visible, title, message, onCancel, onConfirm, cancelText, confirmText }) => {
  ...
  useEffect(() => {
    const backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
      if (visible) {
        // Prevent default back behavior and call cancel handler
        onCancel();
        return true;
      }
      return false;
  ...
  return (
    <Modal
      animationType="none" // Change from "fade" to "none" to prevent animation issues
      transparent={true}
      visible={visible}
      onRequestClose={() => {
        // Handle back button press on Android
        console.log('Modal request close - calling onCancel');
        onCancel();
      }}
    >
